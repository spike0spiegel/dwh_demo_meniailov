...
--SCD1
--
--CREATE OR REPLACE FUNCTION bl_cl.promotions_scd1_3nf()
--RETURNS TRIGGER
--AS $$
--DECLARE
--	v_update_dt DATE;
...
--				WHERE 	p.promotion_src_id = NEW.promotion_src_id AND
--				LOWER(p.source_system) = LOWER(new.source_system) AND
--				p.promotion_name <> NEW.promotion_name) THEN
--	--v_update_dt
--	EXECUTE 'SELECT MAX('|| v_date_name ||')::DATE
--			FROM ' || quote_ident(NEW.source_system) || '.' || quote_ident(NEW.source_entity) || '
...
--			AND is_processed IS TRUE'
--			INTO v_update_dt
--	USING NEW.promotion_src_id;
--	UPDATE bl_3nf.ce_promotions p
--	SET p.promotion_name = NEW.promotion_name,
--		p.ta_update_dt = v.update_dt; 
--	RETURN NULL;
--	ELSE RETURN NEW;
--	END IF;				
--END;
...
--LANGUAGE plpgsql;
--
--DROP TRIGGER IF EXISTS trigger_promotions_scd1_nf ON bl_3nf.ce_promotions;
--CREATE TRIGGER trigger_promotions_scd1_nf
--BEFORE INSERT ON bl_3nf.ce_promotions
--FOR EACH ROW EXECUTE PROCEDURE bl_cl.promotions_scd1_3nf();
--
--CREATE OR REPLACE FUNCTION bl_cl.promotions_scd1_dm()
--RETURNS TRIGGER
--AS $$
--DECLARE
--	v_update_dt DATE;
--	v_date_name VARCHAR;
--BEGIN
--	UPDATE bl_dm.dim_promotions p
--	SET p.promotion_name = NEW.promotion_name,
--		p.ta_update_dt = NEW.update_dt
--	WHERE p.promotion_src_id = NEW.promotion_id; 		
--END;
--$$
--LANGUAGE plpgsql;
--
--DROP TRIGGER IF EXISTS trigger_promotions_scd1_dm ON bl_3nf.ce_promotions;
--CREATE TRIGGER trigger_promotions_scd1_dm
--AFTER UPDATE ON bl_3nf.ce_promotions
--FOR EACH ROW WHEN (pg_trigger_depth() < 1) EXECUTE PROCEDURE bl_cl.promotions_scd1_dm();
--
--
--
